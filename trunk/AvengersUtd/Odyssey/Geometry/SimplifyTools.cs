    ...
    public static class SimplifyTools
    {
        ...
        public static Vertices CollinearSimplify(Vertices vertices, double collinearityTolerance)
        {
            ...
            for (int i = 0; i < vertices.Count; i++)
            {
                ...
                if (MathHelper.Collinear(ref prev, ref current, ref next, collinearityTolerance))
                    continue;
        ...
        private static double DistancePointLine(Vector2D p, Vector2D A, Vector2D B)
        {
            ...
            if (Math.Abs(A.X - B.X) < MathHelper.EpsilonD && Math.Abs(A.Y - B.Y) < MathHelper.EpsilonD)
        ...
        public static Vertices ReduceByArea(Vertices vertices, double areaTolerance)
        {
            ...
            for (int index = 0; index < vertices.Count; ++index, v2 = v3)
            {
                ...
                Vector2D.Cross(ref v1, ref v2, out old1);
                Vector2D.Cross(ref v2, ref v3, out old2);
                Vector2D.Cross(ref v1, ref v3, out new1);
