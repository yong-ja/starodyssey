...
namespace AvengersUtd.MultiversalRuleSystem.Space.CelestialObjects
{
    ...
    public class StellarFeatures
    {
        ...
        public StellarFeatures(string name, StarType type, double magnitude,
            double mass, double luminosity, double temperature, double radius, double semiMajorAxis, double eccentricity, double orbitalPeriod,
            double density,double age, string spectralClass)
        {
            this.name = name;
            this.type = type;
            this.mass = mass;
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine(string.Format("{0}: {1} Age:{2:F2} GY", name, spectralClass, age));
            ...
            sb.AppendLine(string.Format("R: {0:F2}\n\n", radius));
            return sb.ToString();
        }
    }
}