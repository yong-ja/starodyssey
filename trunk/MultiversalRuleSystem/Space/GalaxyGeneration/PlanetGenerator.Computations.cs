...
namespace AvengersUtd.MultiversalRuleSystem.Space.GalaxyGeneration
{
    public partial class PlanetGenerator
    {
            ...
            out bool resonance)
        {
            ...
            {
                resonance = true; // had been only w/large eccentricity, but...
                if (eccentricity > 0.1)
                {
        ...
        /// <summary>
        /// Computes the Tidal Forces acting on a satellite by the parent planet as a
        /// proportion of the Tidal Forces acting on Io by Jupiter.
        /// </summary>
        /// <param name="massPlanet">The mass of the primary body (the planet), in Earth Masses.</param>
        /// <param name="massSatellite">The mass of the satellite whose forces are acting on, in Earth Masses.</param>
        /// <param name="radiusSatellite">The radius of the satellite, in Km.</param>
        /// <param name="distance">The distance between their centers, in AU.</param>
        /// <returns>The Tidal Forces acting on the satellite, as a proportion on those acting
        /// on Io by Jupiter.</returns>
        public static double ComputeTidalForcesOnSatelliteIo(
            double massPlanet, double massSatellite, double radiusSatellite, double distance)
        {
            const double JupiterIoDistanceAU = 0.002819;
            const double JupiterMass = 317.8;
            const double IoMass = 0.015;
            const double IoRadius = 1821.3;

            return (massPlanet * massSatellite * (radiusSatellite / PhysicalConstants.EarthRadiusInKm) * Math.Pow(JupiterIoDistanceAU, 3.0)) /
                   ((Math.Pow(distance, 3.0) * JupiterMass * IoMass * (IoRadius / PhysicalConstants.EarthRadiusInKm)));
        }

        /// <summary>
        /// Computes the Tidal Forces acting on a satellite by the parent planet as a
        /// proportion of the Tidal Forces acting on Luna by Terra.
        /// </summary>
        /// <param name="massPlanet">The mass of the primary body (the planet), in Earth Masses.</param>
        /// <param name="massSatellite">The mass of the satellite whose forces are acting on, in Earth Masses.</param>
        /// <param name="radiusSatellite">The radius of the satellite, in Km.</param>
        /// <param name="distance">The distance between their centers, in AU.</param>
        /// <returns>The Tidal Forces acting on the satellite, as a proportion on those acting on Luna by Terra.</returns>
        public static double ComputeTidalForcesOnSatelliteMoon(
            double massPlanet, double massSatellite, double radiusSatellite, double distance)
        {
            const double EarthMoonDistanceAU = 0.00257;
            const double EarthMass = 1;
            const double LunaMass = 0.0123;
            const double LunaRadius = 1737.1;

            return (massPlanet * massSatellite * (radiusSatellite / PhysicalConstants.EarthRadiusInKm) * Math.Pow(EarthMoonDistanceAU, 3.0)) /
                   ((Math.Pow(distance, 3.0) * EarthMass * LunaMass * (LunaRadius / PhysicalConstants.EarthRadiusInKm)));
        }

        /// <summary>
        /// Computes the Tidal Forces acting on a planet by the parent star as a
        /// proportion of the Tidal Forces acting on Terra by Sol.
        /// </summary>
        /// <param name="massStar">The mass of the primary body (the star), in Solar Masses.</param>
        /// <param name="massPlanet">The mass of the planet whose forces are acting on, in Earth Masses.</param>
        /// <param name="radiusPlanet">The radius of the planet, in Km.</param>
        /// <param name="distance">The distance between their centers, in AU.</param>
        /// <returns>The Tidal Forces acting on the satellite, as a proportion on those acting on Terra by Sol.</returns>
        public static double ComputeTidalForcesOnPlanet(
            double massStar, double massPlanet, double radiusPlanet, double distance)
        {
            const double SolEarthDistanceAU = 1;
            const double SolMass = PhysicalConstants.SolarMassInEarthMasses;
            const double TerraMass = 1;
            const double TerraRadius = PhysicalConstants.EarthRadiusInKm;

            massStar *= PhysicalConstants.SolarMassInEarthMasses;

            return (massStar * massPlanet * (radiusPlanet / PhysicalConstants.EarthRadiusInKm) * Math.Pow(SolEarthDistanceAU, 3.0)) /
                   ((Math.Pow(distance, 3.0) * SolMass * TerraMass * (TerraRadius / PhysicalConstants.EarthRadiusInKm)));
        }
