...
namespace AvengersUtd.MultiversalRuleSystem.Space.GalaxyGeneration
{
    public partial class PlanetGenerator
    {
        ...
        /// <returns>Average density in Earth densities.</returns>
        ...
        public static double ComputeRmsVelocity(double molecularWeight, double orbitalRadius)
        {
            double exosphericTemp;

            exosphericTemp = PhysicalConstants.EarthExosphereTemperature/(orbitalRadius*orbitalRadius);
            return (Math.Sqrt((3.0*PhysicalConstants.MolarGasConstant*exosphericTemp)/
                              molecularWeight));
        }
        ...
        public static double ComputeMolecularWeightRetained(double escapeVelocity)
        {
            if (escapeVelocity == 0.0)
                return 0.0;
            else
            {
                return (3.0 * PhysicalConstants.MolarGasConstant * 
                    PhysicalConstants.EarthExosphereTemperature) /
                    (Math.Pow((escapeVelocity/PhysicalConstants.GasRetentionThreshold) /
                PhysicalConstants.CmPerMeter,2));
